...
class FieldLockTest < ActiveSupport::TestCase
  test 'can add field locks to a model' do
    ...
    event.locked_fields = %i[title description]
    ...
    assert_equal %i[title description].sort, event.locked_fields.sort
  ...
  test 'does not add duplicate field locks' do
    ...
    event.locked_fields = %i[title description description]
