...
class EventsControllerTest < ActionController::TestCase
  ...
  test 'should create online event for user' do
    ...
    refute_nil assigns(:event), 'event is nil'
    refute_nil assigns(:event).duration, 'duration is nil'
    assert_equal @event.duration, assigns(:event).duration, 'duration not matched'
    refute_nil assigns(:event).recognition, 'recognition is nil'
    assert_equal @event.recognition, assigns(:event).recognition, 'recognition not matched'
    refute_nil assigns(:event).learning_objectives, 'learning objectives is nil'
  ...
  test 'should create event for admin' do
    ...
    assert_difference('Event.count') do
      post :create,
           params: { event: { description: @event.description, title: @event.title,
    ...
    assert_difference('Event.count') do
      post :create, params: {
        ...
        event: { title: event_title, url: 'http://horse.com',
                 description: 'All about horses' }.merge(@mandatory_fields),
        format: 'json'
      }
    end
  ...
  test 'should provide an ics file' do
    ...
    refute @event.all_day?, 'not an all day event'
    ...
    assert_difference('ExternalResource.count', 1) do
      patch :update, params: {
        ...
        event: {
          ...
          external_resources_attributes: { '1' => { title: 'Cool link', url: 'https://tess.elixir-uk.org/',
                                                    _destroy: '0' } }
        }
      }
    end
    ...
    assert_difference(-> { suggestion.scientific_topic_links.count }, -1) do
      assert_difference(-> { @event.scientific_topic_links.count }, 1) do
        assert_difference('EditSuggestion.count', -1) do
          post :add_term,
               params: { id: @event.id, field: 'scientific_topics', uri: 'http://edamontology.org/topic_0622' }
        end
      end
    end
