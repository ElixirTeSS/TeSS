...
class CollaborationsControllerTest < ActionController::TestCase
  ...
  test 'should add collaborator' do
    ...
    assert_difference('Collaboration.count', 1) do
      post :create,
           params: { format: :json, workflow_id: @workflow.id,
                     collaboration: { user_id: users(:non_collaborative_user).id } }
    end
  ...
  test 'should not add duplicate collaborator' do
    ...
    assert_no_difference('Collaboration.count') do
      post :create,
           params: { format: :json, workflow_id: @workflow.id,
                     collaboration: { user_id: users(:collaborative_user).id } }
    end

    assert_response :unprocessable_entity
    assert_includes JSON.parse(@response.body)['errors']['user'].join, 'already a collaborator'
  end
