...
namespace :tess do
  ...
  task download_images: :environment do
    ...
    begin
      [Collection, ContentProvider, StaffMember].each do |klass|
        downloadable = klass.all.select { |o| o.image_url.present? && !o.image? }
  ...
  task expire_sessions: :environment do
    ...
    puts "Deleting sessions older than #{max_age.inspect} at #{Time.zone.now}:"
  ...
  task process_subscriptions: :environment do
    ...
    puts "Processing #{subs.count} subscriptions at #{Time.zone.now}: "
    subs.each do |sub|
      sub.process
      print '.'
    end
  ...
  task reset_subscriptions: :environment do
    ...
    puts "Resetting #{subs.count} subscriptions at #{Time.zone.now}:"
    subs.each do |sub|
      sub.reset_due
      print '.'
    end
  ...
  task fetch_spdx: :environment do
    old_licenses = YAML.safe_load(File.read(File.join(Rails.root, 'config', 'dictionaries', 'licences_old.yml')))
