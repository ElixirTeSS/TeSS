...
module Ingestors
  class OsciIngestor < Ingestor
    ...
    def process_osci(_url)
      ...
      (1..12).each do |i|
        sleep(1) unless Rails.env.test? && File.exist?('test/vcr_cassettes/ingestors/osci.yml')
        ...
        event_page.each do |event_data|
          ...
          beep.each do |boop|
            ...
            if el2.css("div[class='time-block']")[0].css("span[class='event-time dtstart']").count.positive?
              ...
              event.end = if el2.css("div[class='time-block']")[0].css("span[class='event-time dtend']").count.positive?
                            Time.zone.parse(el2.css("div[class='time-block']")[0].css("span[class='event-time dtend']")[0].text.strip)
                          else
                            event.start
                          end
            end
