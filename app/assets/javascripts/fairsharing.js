var Fairsharing = {
  ...
  search: function () {
    $('.loading_image').show();
    Fairsharing.queryAPI(
      Fairsharing.allTypeURL() +
        '?' +
        Fairsharing.queryParameter() +
        '&' +
        Fairsharing.typeParameter()
    );
  },
  nextPage: function () {
    ...
    json.results.forEach(function (item, index) {
      ...
      $('#fairsharing-results').append(
        '' +
          '<div id="' +
          id +
          '" class="col-md-12 col-sm-12 bounding-box" data-toggle="tooltip" data-placement="top" aria-hidden="true" title="' +
          item.description +
          '">' +
          '<h4>' +
          '<i class="fa ' +
          ...
          '</h4>' +
          '<span>' +
          truncateWithEllipses(item.description, 500) +
          '</span>' +
          '<div class="external-links">' +
          '<a class="btn btn-warning" target="_blank" href="' +
          Fairsharing.websiteBaseURL() +
          '/' +
          id +
          '">' +
          'View ' +
    ...
    $.ajax({
      ...
      success: function (result) {
        ...
        $.each(json_object.domains, function (index, domain) {
          $('#' + id + '-topics').append(
            '<span class="btn btn-default keyword-button">' + domain + '</span>'
          );
        });
        ...
        $('#' + id + '-external-links').append(
          '<div>' +
            '<a class="btn btn-success external-button" target="_blank" href="' +
            json_object.homepage +
            '">' +
            'View the ' +
            json_object.name +
            ' homepage ' +
            '<i class="fa fa-external-link"/></a>' +
            '</a>' +
            '<a class="btn btn-warning external-button" target="_blank" href="' +
            Fairsharing.websiteBaseURL() +
            '/' +
...
document.addEventListener('turbolinks:load', function () {
  ...
  $('#fairsharing-results').on(
    'click',
    '.associate-tool',
    Fairsharing.associateTool
  );
  $('#external-resources').on(
    'change',
    '.delete-external-resource-btn input.destroy-attribute',
    ExternalResources.delete
  );
