...
class User < ApplicationRecord
  ...
  CREATED_RESOURCE_TYPES = [:events, :materials, :workflows, :content_providers, :sources, :collections, :nodes].freeze
  has_many :materials
  ...
  validate :consents_to_processing, on: :create, unless: lambda { |user|
                                                           user.using_omniauth? || User.current_user.try(:is_admin?)
                                                         }
  ...
  def self.with_created_resources
    joins(:activities_as_owner).where(activities: { key: CREATED_RESOURCE_TYPES.map do |t|
                                                           "#{t.to_s.singularize}.create"
                                                         end }).distinct
  end
  ...
  def get_inviter
    unless invited_by_id.nil?
      inviter = User.find_by(id: invited_by_id)
      inviter.username
    end
  end
  ...
  def consents_to_processing
    if processing_consent != '1'
      errors.add(:base,
                 "You must consent to #{TeSS::Config.site['title_short']} processing your data in order to register")
  ...
  def set_username_for_invitee
    self.username = email if !invitation_token.nil? && !email.nil? && username.nil?
  end
end
