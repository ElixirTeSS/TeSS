...
module ApplicationHelper
  ...
  def enabled_icon(record, size = nil)
    return if record.nil? || record.enabled.nil?
  ...
  def currency_by_iso_code(iso_code)
    return unless !iso_code.nil? && !iso_code.blank?
  ...
  def language_options_for_select(priority = priority_languages)
    ...
    I18nData.languages.each do |lang|
      next unless lang && !lang.empty?

      value = lang[1]
      key = lang[0]
      # Rails.logger.debug "language: key[#{key}] value[#{value}]"
      if priority && !priority.empty? && priority.include?(key)
        priors << [value, key]
      else
        others << [value, key]
      end
