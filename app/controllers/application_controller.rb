...
class ApplicationController < ActionController::Base
  ...
  acts_as_token_authentication_handler_for User, except: %i[index show embed calendar check_exists handle_error count
  ...
  before_action :authenticate_user!, except: %i[index show embed calendar check_exists handle_error count redirect]
  ...
  def handle_error(status_code = 500, message = nil)
    ...
    status_code = Rack::Utils::SYMBOL_TO_STATUS_CODE[status_code] || status_code if status_code.is_a?(Symbol) # Convert :forbidden, :not_found, etc. to 403, 404 etc.
    ...
    respond_to do |format|
      ...
      format.json { render json: { error: { message:, code: status_code } }, status: status_code }
      format.json_api { render json: { error: { message:, code: status_code } }, status: status_code }
    end
  end
  ...
  def feature_enabled?(feature = controller_name)
    return unless TeSS::Config.feature.key?(feature) && !TeSS::Config.feature[feature]
  ...
  def set_current_user
    ...
    return unless TeSS::Config.sentry_enabled?
