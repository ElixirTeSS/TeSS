...
class CuratorController < ApplicationController
  ...
  def action_count_for(action)
    PublicActivity::Activity.where(key: action).group_by(&:owner).sort_by { |_user, logs| -logs.count }.transform_values(&:count)
  end

  def check_curator
    return if current_user && (current_user.is_admin? || current_user.is_curator?)
