...
class ConvertScientificTopicSerializedTextFieldsToArrays < ActiveRecord::Migration[4.2]
  def up
    fields = %i[synonyms definitions parents consider
    ...
    ScientificTopic.transaction do
      ScientificTopic.all.each do |e|
        fields.each do |field|
          e.update_column("#{field}2".to_sym, YAML.load(e.send(field))) unless e.send(field).blank?
          print '.'
        end
      end
    end
  ...
  def down
    ...
    ScientificTopic.transaction do
      ScientificTopic.all.each do |e|
        fields.each do |field|
          e.update_column("#{field}2".to_sym, e.send(field).to_yaml) unless e.send(field).blank?
          print '.'
        end
      end
    end
